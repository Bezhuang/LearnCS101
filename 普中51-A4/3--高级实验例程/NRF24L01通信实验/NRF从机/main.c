...
void blink(char i) //流水灯函数
{
	P2=~0x01;
	delay_ms(450); //大约延时450ms	
		for(i=0;i<8;i++)
		{
			P2=~(0x01<<i);	 //将1右移i位，然后将结果赋值到P0口
			delay_ms(450);
		}
	P2=0Xff;
}
...
void main(void)
{
    ...
    while(1)
    {
        ...
        SPI_RW_Reg(WRITE_REG + STATUS, sta);  // 清楚RX_DS中断标志
